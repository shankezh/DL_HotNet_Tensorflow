#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Created by Incetpion_V3 on 19-4-11
...
def run():
    ...
    BATCH_SIZE = 32
    EPOCH_NUM = 150
    ITER_NUM = 1500  # 50000 / 100
    LEARNING_RATE_VAL = 0.001

    if utils.isLinuxSys():
        logdir = r'/home/ws/DataSets/cifar/logs/train/inceptionv3'
        model_dir = r'/home/ws/DataSets/cifar/model_file/inceptionv3'
    else:
        model_dir = r'D:\DataSets\cifar\cifar\model_flie\inceptionv3'
        logdir = r'D:\DataSets\cifar\cifar\logs\train\inceptionv3'

    if is_train:
        train_img_batch, train_label_batch = pre_pro.get_cifar10_batch(is_train=True, batch_size=BATCH_SIZE,
                                                                       num_cls=num_cls, img_prob=img_prob)
        test_img_batch, test_label_batch = pre_pro.get_cifar10_batch(is_train=False, batch_size=BATCH_SIZE,
                                                                     num_cls=num_cls, img_prob=img_prob)
                ...
                if is_train:
                    ...
                    for epoch in range(EPOCH_NUM):
                        ...
                        for step in range(1, ITER_NUM + 1):
                            ...
                            _, batch_train_loss, batch_train_acc = sess.run([train_optim, train_loss, train_eval],
                                                                            feed_dict={inputs: batch_train_img,
                                                                                       labels: batch_train_label,
                                                                                       LEARNING_RATE: LEARNING_RATE_VAL,
                                                                                       is_training: is_train})
                            global_step = int(epoch * ITER_NUM + step + 1)

                            print("epoch %d , step %d train end ,loss is : %f ,accuracy is %f ... ..." % (
                            epoch, step, batch_train_loss, batch_train_acc))
                            ...
                            if step % 100 == 0:
                                ...
                                for ac_count in range(ac_iter):
                                    ...
                                    test_loss, test_accuracy = sess.run([train_loss, train_eval],
                                                                        feed_dict={inputs: batch_test_img,
                                ...
                                if ac_mean >= max_acc:
                                    max_acc = ac_mean
                                    saver.save(sess, model_dir + '/' + 'cifar10_{}_step_{}.ckpt'.format(str(epoch),
                                                                                                        str(step)),
                                               global_step=step)
